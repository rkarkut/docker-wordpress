 ...
 * Class GR_Int_Widget.
 *
 * GetResponse Widget class.
 */
class GR_Int_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			'getresponse-widget',
			__( 'GetResponse Web Form (New)', 'Gr_Integration' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( isset( $web_forms[ $webform_id ] ) ) {
			...
			if ( false === empty( $api_key ) ) {
			    ...
			    try {
                    ...
                    if ('old' == $version) {
                        $w = gr()->api->get_web_form($webform_id);
                    } else {
                        $w = gr()->api->get_form($webform_id);
                    }
				...
				$webform['scriptUrl'] = isset( $w->scriptUrl ) ? $w->scriptUrl : null;
				$webform['status']    = isset( $w->status ) ? $w->status : null;
		...
		if ( ! empty( $webform )
		     && isset( $webform['scriptUrl'] )
		     && in_array( $webform['status'], array( 'enabled', 'published' ) )
		) {
...
//			$webform['href'] = $this->replaceHttpsToHttpIfSslOn( $webform['href'] );
	...
	public function update( $new_instance, $old_instance ) {
		...
		try {
            $webform = ($instance['version'] == 'old') ? gr()->api->get_web_form($instance['select']) : gr()->api->get_form($instance['select']);
        } catch (Unauthorized_Request_Exception $e) {
            gr()->disconnect_integration();
        }
		$web_forms = json_decode( gr_get_option( 'web_forms' ), true );
